...
export const getNotifications = asyncHandler(async (req, res, next) => {
  ...
  if (role === "Admin") {
    query.type = { $in: ["request"] }; // adjust if needed
  } else {
    query.user = userId;
  }
  ...
  const highPriorityMessage = notifications.filter(
    (n) => n.priority === "high",
  );
  ...
  const thisWeekNotification = notifications.filter((n) => {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      highPriorityMessages: highPriorityMessage.length, 
      thisWeekNotifications: thisWeekNotification.length, 
    },
  });
});
...
export const deleteNotification = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "delete Notification successsfully",
  });
});
